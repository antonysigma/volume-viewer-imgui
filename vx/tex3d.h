...
void
loadTex3D(unsigned char *vptr, int isInit) { /* create single 3D texture */
    ...
    vp = vptr;
    for (i = 0; i < nBytes; i++) *rgba++ = gRGBAlut[*vp++];
    glTexImage3DEXT(GL_TEXTURE_3D_EXT, 0, GL_RGBA8, gDimXYZ[0], gDimXYZ[1], gDimXYZ[2], 0, GL_RGBA,
                    GL_UNSIGNED_BYTE, voxels);
    free(voxels);
} /* loadTex3D() */

void
drawGL3D() { /* volume render using a single 3D texture */
    // printf("Draw3D\n");
    ...
    renderQuality =
        1.7f * 1.0f / ((float)gDimXYZ[0] + (float)gDimXYZ[1] + (float)gDimXYZ[2]) * gQuality;
