 ...
 *      Rotation assumes roughtly canonical orientation (with spatial matrix like [1 0 0; 0 1 0; 0 0
 *1]) 4.) Limited opacity correction: thinner slices (e.g. higher quality) should be more opaque
 */
...
int gQuality =
    1; /*1=best, 2=fast/mid-quality (half sampling), 4=fastest/poor-quality (quarter sampling)*/
...
float
swapFloat(float in) { /*convert endian of 32-bit single precision float*/
    float out;
    char *inChar = (char *)&in;
...
int
swapInt(int in) { /*convert endian of 32-bit integer*/
    int out;
    char *inChar = (char *)&in;
    char *outChar = (char *)&out;
    outChar[0] = inChar[3];
...
short
swapShort(short in) { /*convert endian of 16-bit integer*/
    short out;
    char *inChar = (char *)&in;
    char *outChar = (char *)&out;
    outChar[0] = inChar[1];
...
int
isPowerOfTwo(unsigned int x) { /*is input a PoT (2,4,8,16,32...)?*/
    return ((x != 0) && !(x & (x - 1)));
}

int
isGz(char **argv) { /*detect if file is gzip format, e.g. file.nii.gz*/
    int found = 0;
    char *p = strrchr(argv[1], '.');
    if (p) found = strcmp(p, ".gz") == 0;
    return found;
}

void
readHeader(char **argv, struct nifti_1_header *hdr) { /*read NIfTI format header*/
    int i;
    FILE *fp;
    ...
    if (swapInt(hdr->sizeof_hdr) == 348) {
        ...
        if (hdr->bitpix != 8)
            printf(
                "Support for foreign-endian images with >8bpp will require swapping the image "
                "data\n");
    }
    ...
    if (!isPowerOfTwo(hdr->dim[1]) || !isPowerOfTwo(hdr->dim[2]) || !isPowerOfTwo(hdr->dim[3]))
        printf(
            "image dimensions are not a power of two. Older graphics will not support this! "
            "Solution: pad image.\n");
#endif
    gDimXYZ[0] = hdr->dim[1];
    gDimXYZ[1] = hdr->dim[2];
...
void
drawGL() { /* volume render using a single 3D texture */
    drawGL3D();
}

void
loadTex(unsigned char *vptr, int isInit) { /* volume render using a single 3D texture */
...
void
readVoxelData(char **argv) { /*load image data*/
...
drawScene(void) {
    ...
    while (gAzimuth < 0) gAzimuth += 360;
    while (gAzimuth >= 360) gAzimuth -= 360;
    while (gElevation > 90) gElevation = 90;
    while (gElevation < -90) gElevation = -90;
...
void
mouseDown(int button, int state, int x, int y) {
    gMousePosX = x;
    gMousePosY = y;
}
