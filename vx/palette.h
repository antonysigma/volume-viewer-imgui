...
uint32_t
lerprgb(uint32_t lo, uint32_t hi, int loindex, int hiindex,
        int tarindex) {  // linearly interpolate to blend RGBA colors
    int i;
    uint32_t ret;
    ...
    for (i = 0; i < 4; i++) pret[i] = (plo[i] + frac * (phi[i] - plo[i]));
    return ret;
}
...
double
round_c99(double x) { /* round is not part of classic C used by MPISpro */
    return (x >= 0.0) ? floor(x + 0.5) : ceil(x - 0.5);
}

struct RGBAnode
makeRGBAnode(unsigned char r, unsigned char g, unsigned char b, float a, int inten) {
    struct RGBAnode ret;
    float ax = a;
    if (ax < 0) ax = 0;
    if (ax > 255) ax = 255;
...
void
filllut(struct RGBAnode loNode, struct RGBAnode hiNode, uint32_t* lut) {
    int i, mn, mx;
...
void
createlutX(int colorscheme, uint32_t* lut) {
    float oc;
    int i;
    ...
    for (i = 1; i < numNodes; i++) filllut(nodes[i - 1], nodes[i], lut);
    return;
}

void
changeLUT(int brightness, int contrast, int table, int isInit) {
    ...
    if (!isInit)
        printf("palette %d with brightness (center) %d and contrast (width) %d \n", gColorTable,
               gWindowCenter, gWindowWidth);
    /* IN THEORY for SGI we only need to update the color palette, we do not need to modify the
     * textures */
    /*
    #ifdef __sgi
       glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
       glColorTableSGI( GL_TEXTURE_COLOR_TABLE_SGI, GL_RGBA8_EXT, 256, GL_RGBA, GL_UNSIGNED_BYTE,
    gRGBAlut); glEnable(GL_TEXTURE_COLOR_TABLE_SGI); if (isInit) { loadTex(vptr, isInit);
                   free(vptr);
            }
    #else */
    loadTex(vptr, isInit);
